    ...
    try {
        ...
        if (message.channel.id == consolechannel) {
            ...
            if (bot_json["Translate_State"] == true) {
                ...
                if (message.channel.id != bot_json["Translate_en"] && message.channel.id == bot_json["Translate_zh_CN"]) {
                    translate(message.content, { to: 'en' }).then(Tres => {
                        C_send(bot_json["Translate_en"], message.author.username + " >> " + Tres.text);
                    }).catch(err => {
                        E_error(":name_badge: Error: 3-5-0016", err)
                    });
                }
                if (message.channel.id != bot_json["Translate_zh_CN"] && message.channel.id == bot_json["Translate_en"]) {
                    translate(message.content, { to: 'zh-TW' }).then(Tres => {
                        C_send(bot_json["Translate_zh_CN"], message.author.username + " >> " + Tres.text);
                    }).catch(err => {
                        E_error(":name_badge: Error: 3-5-0016", err)
                    });
                }
        ...
        fs.readFile(string_path, function (error, data) {
            ...
            fs.readFile(bot_path, function (error, data) {
                if (error) {
                    ...
                    if (check != "") C_send(consolechannel, ":placard: Translate_zh_CN [" + bot_json["Translate_zh_CN"] + "]");
                    ...
                    if (bot_json["Translate_State"] != false && (bot_json["Translate_zh_CN"] == "Channel_ID" || bot_json["Translate_en"] == "Channel_ID")) {
...
function SET(x) {
    try {
        ...
        } else if (x[0] == "Translate_zh_CN") {
            if (Number.isInteger(Number(x[1])) == true) {
                bot_json_cache["Translate_zh_CN"] = x[1]
                C_send(consolechannel, ":white_check_mark: Translate_zh_CN 已成功設定為 [" + x[1] + "]\n:warning: 使用 Reload 來套用新設定");

            } else if (x[1] == "clear") {
                bot_json_cache["Translate_zh_CN"] = "Channel_ID"
                bot_json_cache["Translate_zh_CN_State"] = false
                C_send(consolechannel, ":white_check_mark: Translate_zh_CN 已成功清除\n:warning: 已關閉 Channel_Adjustment_Notification 功能\n:warning: 使用 Reload 來套用新設定");
            } else {
                C_send(consolechannel, ":warning: 未知的附屬指令");
            }
